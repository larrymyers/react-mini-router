...
module.exports = {
    ...
    componentDidMount: function() {
        ...
        _events.push(event.listenOn(ReactDOM.findDOMNode(this), 'click', this.handleClick));

        if (this.state.useHistory) {
            _events.push(event.listenOn(window, 'popstate', this.onPopState));
        } else {
            if (window.location.hash.indexOf('#!') === -1) {
                window.location.hash = '#!/';
            }

            _events.push(event.listenOn(window, 'hashchange', this.onPopState));
        }
    },

    componentWillUnmount: function() {
        this._events.forEach(function(removeListener) {
           removeListener();
        });
    },
